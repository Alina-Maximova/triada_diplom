...
import { CreateTaskData, Location as LocationType } from '@/types';
...
interface FormData {
  ...
  phone: string;
  customer: string;
  addressNote: string; // Примечание к адресу - ОПЦИОНАЛЬНО
...
export default function CreateTaskScreen() {
  ...
  const {
    ...
    defaultValues: {
      ...
      phone: '',
      customer: '',
      addressNote: '', // Опциональное поле, может быть пустым
      start_date: null,
      due_date: null,
      startTime: new Date(2026, 0, 1, 9, 0),
      dueTime: new Date(2026, 0, 1, 18, 0),
      location: null,
    },
    mode: 'onChange',
  });
  ...
  const addressNote = watch('addressNote'); // Опциональное поле

  // Убираем addressNote из проверки обязательных полей
  const isFormValid =
    title?.trim() &&
    customer?.trim() &&
    phone?.trim() &&
    ...
    // addressNote?.trim() && // УБИРАЕМ эту проверку - поле не обязательное
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const submitData: CreateTaskData = {
        ...
        addressNote: data.addressNote?.trim() || '', // Используем пустую строку если undefined/null
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        {/* Примечание к адресу - ОПЦИОНАЛЬНО */}
        <Controller
          control={control}
          name="addressNote"
          // НЕТ правил валидации - поле не обязательное
          render={({ field: { onChange, value } }) => (
            <TextInput
              label="Примечание к адресу (опционально)"
              value={value}
              onChangeText={onChange}
              style={styles.input}
              mode="outlined"
              ...
              numberOfLines={2}
              maxLength={500}
              placeholder="Уточните детали адреса, подъезд, этаж, код домофона и т.д."
            />
          )}
        />
